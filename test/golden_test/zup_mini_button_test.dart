...
void main() {
  ...
  zGoldenTest("When the onPressed is null, the button should be in the disabled state",
      goldenFileName: "zup_mini_button_disabled", (tester) async {
    return tester.pumpDeviceBuilder(await goldenBuilder(onPressed: null));
  });

  zGoldenTest("When the onPressed is not null, it should be in the enabled state",
      goldenFileName: "zup_mini_button_enabled", (tester) async {
    return tester.pumpDeviceBuilder(await goldenBuilder(onPressed: () {}));
  });

  zGoldenTest("When hovering the button, and it's enabled, the button should be in the hovered state",
      goldenFileName: "zup_mini_button_hover", (tester) async {
    await tester.pumpDeviceBuilder(await goldenBuilder(onPressed: () {}, title: "Hovered"));

    await tester.hover(find.byType(ZupMiniButton));
    await tester.pumpAndSettle();
  });

  zGoldenTest("When hovering the button, and it's not enabled, the button should not be in the hovered state",
      goldenFileName: "zup_mini_button_hover_disabled", (tester) async {
